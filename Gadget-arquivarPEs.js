...
( function ( $, mw ) {
	...
	ape.getWikiText = async function ( title ) {
		...
		const q = data.query;
		const id = q && q.pageids && q.pageids[ 0 ];
		const pg = id && q.pages && q.pages[ id ];
		const rv = pg && pg.revisions;
		if ( rv && rv[ 0 ] && rv[ 0 ][ '*' ] ) {
			return rv[ 0 ][ '*' ];
		} else {
	...
	ape.startArchive = async function ( pfdwText ) {
		const notFinishedRegex = /\{\{Nomeação não concluída\|(\d*)(|\|prog=.*)\}\}/g,
	...
	ape.addArchiveLink = function ( element ) {
		const $archivePFD = $( '<a href="#">Arquivar</a>' ).on( 'click', function ( e ) {
			e.preventDefault();
			...
			ape.nicePrompt(
				...
				async function ( result ) {
					...
					const PEtext = await ape.getWikiText( mw.config.get( 'wgPageName' ) );
	...
	ape.removeTemplate = async function ( page ) {
		...
		const wtext = await ape.getWikiText( page );
		await ape.editPage(
			page,
	...
	$( '.PEferramentas' ).each( function () {
		ape.addArchiveLink( this );
	} );
